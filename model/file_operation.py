...
def writeTrainedTuples(trained_tuples):
    with open("model.txt", "w", encoding="utf-8") as trainData:
...
def writeCalculatedValues():
    with open("values.txt", "w", encoding="utf-8") as calculateInfo:
...
def readCalculatedValues():
    with open("values.txt", "r", encoding="utf-8") as calculateInfo:
...
def readTrainedModelFile():
    with open("model.txt", "r") as trainData:
